    ...
    <Card className ="bg-[#e6d5bc] text-[#60432f] border border-[#c7a26b]">
      <CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
        ...
        <Select value={metric} onValueChange={setMetric}>
          ...
          <SelectContent className="rounded-xl bg-[#e6d5bc] text-[#60432f] ">
            {
              cols.map(col => {
                if (col!= "year") return (
                  <SelectItem value= {col} className="bg-[#e6d5bc] text-[#60432f] hover:text-[#e6d5bc] aria-[selected=true]:text-[#e6d5bc] "> {col} </SelectItem>
                )
              })
            }
          </SelectContent>
      ...
      <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
        ...
        >
          <BarChart accessibilityLayer data={filteredData}>
            ...
            />
            <Bar dataKey={"metric"} radius={8}  fill = "#957d67"/>
          </BarChart>
        </ChartContainer>
      </CardContent>
    </Card>
